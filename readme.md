...
- **測試是否安裝成功：** 請在終端機輸入(CMD/PowerShell/Terminal/命令提示字元)輸入以下指令並按 <kbd>Enter</kbd>
```Bash
node -v
```
 有出現版本號即表示安裝成功。
...
2. **安裝工具：**
   ...
   npm i
   ```
   打完此指令後會跑一點點時間，並且會增加一個 `node_modules` 的資料夾，這是正常現象。
...
>[!IMPORTANT]
免責聲明： 本專案僅供個人學習與娛樂使用。請勿用於任何商業用途。 若有違反部分遊戲的使用條款，請自行承擔風險。